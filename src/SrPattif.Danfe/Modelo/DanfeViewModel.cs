...
namespace DanfeSharp.Modelo
{
    ...
    public class DanfeViewModel
    {
        ...
        public virtual String TextoRecebimento
        {
            get
            {
                return $"Recebemos de {Emitente.RazaoSocial} os produtos e/ou serviços constantes na Nota Fiscal Eletrônica indicada {(Orientacao == Orientation.Portrait ? "abaixo" : "ao lado" )}. Emissão: {DataHoraEmissao.Formatar()} Valor Total: R$ {CalculoImposto.ValorTotalNota.Formatar()} Destinatário: {Destinatario.RazaoSocial}";
            }
        }
        ...
        public virtual String TextoAdicional()
        {
            ...
            if (!String.IsNullOrEmpty(Pedido) && !Utils.StringContainsAccessKey(InformacoesComplementares, "Pedido", Pedido))
                sb.AppendChaveValor("Pedido", Pedido);

            if (!String.IsNullOrEmpty(Contrato) && !Utils.StringContainsAccessKey(InformacoesComplementares, "Contrato", Contrato))
        ...
        public Boolean IsRetrato => Orientacao == Orientation.Portrait;
        public Boolean IsPaisagem => Orientacao == Orientation.Landscape;
