...
namespace DanfeSharp.Modelo
{
    public static class DanfeViewModelCreator
    {
        ...
        /// <returns></returns>
        public static DanfeViewModel CreateFromXmlFile(string xmlFilePath)
        {
            using (StreamReader sr = new StreamReader(xmlFilePath, true))
            {
                return CreateFromXml(sr);
            }
        }
        ...
        public static DanfeViewModel CreateFromXmlFile(Stream stream)
        {
        ...
        private static DanfeViewModel CreateFromXml(TextReader reader)
        {
            ProcNFe nfe = null;
            XmlSerializer serializer = new XmlSerializer(typeof(ProcNFe));
        ...
        public static DanfeViewModel CreateFromXml(ProcNFe procNfe)
        {
            ...
            model.Orientacao = ide.tpImp == 1 ? Orientation.Portrait : Orientation.Landscape;
            ...
            if (infNfe.cobr != null)
            {
                ...
                {
                    var duplicates = new DuplicataViewModel();
                    duplicates.Numero = item.nDup;
                    duplicates.Valor = item.vDup;
                    duplicates.Vecimento = item.dVenc;

                    model.Duplicatas.Add(duplicates);
                }
            }
